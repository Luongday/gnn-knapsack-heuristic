...
def mark(msg: str) -> None:
    print(f"[gnn-DP-EVAL] {msg}", flush=True)
...
def _default_model_path() -> Path:
    return Path(__file__).resolve().parents[2] / "results" / "GNN" / "gnn_best.pt"

def _default_out_csv() -> Path:
    return Path(__file__).resolve().parents[2] / "results" / "GNN" / "gnn_dp_eval_results.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--decode_strategy", type=str, default="dp_subset",
                        choices=["greedy_prob", "greedy_ratio", "dp_subset",
                                 "beam_search", "diversified_beam"],
                        help="Decode strategy (default dp_subset for this script)")
...
def main() -> None:
    ...
    for idx, path in enumerate(files):
        ...
        x_hat = _decode(probs, W_orig, float(C),
